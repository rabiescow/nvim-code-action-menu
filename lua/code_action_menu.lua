...
local function open_code_action_menu(options)
    ...
    if #all_actions == 0 then
        ...
        menu_window_instance:open({window_stack = {anchor_window_instance}})
    end
end

local function update_details_window(selected_action)
    if vim.g.code_action_menu_show_details == nil or
        vim.g.code_action_menu_show_details then
        if details_window_instance == nil then
...
local function update_diff_window(selected_action)
    if vim.g.code_action_menu_show_diff == nil or
        vim.g.code_action_menu_show_diff then
        if diff_window_instance == nil then
...
local function execute_selected_action()
    ...
    if selected_action:is_disabled() then
        vim.api.nvim_echo({{'Can not execute disabled action!'}, {'ErrorMsg'}},
                          false, {})
    else
...
local function select_line_and_execute_action(line_number)
    ...
    vim.api.nvim_win_set_cursor(menu_window_instance.window_number,
                                {line_number, 0})
