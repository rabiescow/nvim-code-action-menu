...
local function unpack_result_and_error(server_data, client_id)
    ...
    elseif type(server_data) == 'table' and server_data.err ~= nil and client then
        ...
        elseif type(server_data.err) == 'table' and
            type(server_data.err.message) == 'string' then
...
local function resolve_code_action(client_id, code_action_object)
    local client = vim.lsp.get_client_by_id(client_id)
    if client then
        local response = client:request_sync('codeAction/resolve',
                                             code_action_object)
        local action_object, error =
            unpack_result_and_error(response, client_id)

        if error then vim.api.nvim_echo({error, "ErrorMsg"}, true, {}) end
...
local function parse_object_as_action(code_action_object)
    if type(code_action_object) == 'table' and type(code_action_object.command) ==
        'string' then
        return Command:new(code_action_object)
    elseif type(code_action_object) == 'table' and
        (type(code_action_object.edit) == 'table' or
            type(code_action_object.command) == 'table') then
        return CodeAction:new(code_action_object)
    else
        local error =
            'Failed to parse unknown code action or command data structure! Skipped.'
        vim.api.nvim_echo({error, "ErrorMsg"}, true, {})
        return nil
    end
end
...
local function parse_action_data_objects(client_id, all_code_action_objects)
    ...
    for _, code_action_object in ipairs(all_code_action_objects) do
        if type(code_action_object) == 'table' and code_action_object.data ~=
            nil then
            code_action_object = resolve_code_action(client_id,
                                                     code_action_object)
        end

        local action = parse_object_as_action(code_action_object)

        if action ~= nil then table.insert(all_actions, action) end
    end
...
local function request_actions_from_server(client_id, parameters)
    ...
    if client then
        ...
        local response = client:request_sync('textDocument/codeAction',
                                             parameters)
        local action_objects, error = unpack_result_and_error(response,
                                                              client_id)
...
local function request_actions_from_all_servers(options)
    vim.validate('options is table', options, 'table')

    options = options or {}
    local request_parameters = options.use_range and
                                   get_range_request_parameters() or
    ...
    for _, client in pairs(all_clients) do
        local action_data_objects = request_actions_from_server(client.id,
                                                                request_parameters)
        local actions =
            parse_action_data_objects(client.id, action_data_objects)
...
local function iterate_actions_ordered(action_table)
    ...
    return function()
        ...
        if keys[index] then return index, action_table[keys[index]] end
    end
end
