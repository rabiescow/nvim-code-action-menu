...
local function get_text_document_edit_status_icon(status)
    return (status == TextDocumentEditStatusEnum.CREATED and '*' or status ==
               TextDocumentEditStatusEnum.CHANGED and '~' or status ==
               TextDocumentEditStatusEnum.RENAMED and '>' or status ==
               TextDocumentEditStatusEnum.DELETED and '!' or
               error(
                   'Can not get icon unknown TextDocumentEdit status: \'' ..
                       status .. '\''))
end
...
local function get_summary_line_formatted(text_document_edit)
    ...
    local line_number_statistics =
        text_document_edit:get_line_number_statistics()
    local changes = '(+' .. line_number_statistics.added .. ' -' ..
...
local function get_diff_square_counts(text_document_edit)
    ...
    local total_changed_lines_round_to_five =
        total_changed_lines + (modulu_five > 0 and 5 - modulu_five or 0)
    ...
    local squares_for_neutral_fill = 5 - squares_for_added_lines -
...
function DiffWindow:update_virtual_text()
    ...
    for _, text_document_edit in ipairs(workspace_edit.all_text_document_edits) do
        ...
        if square_counts.added > 0 then
            ...
            -- vim.api.nvim_buf_set_extmark(self.buffer_number, self.namespace_id,
            --                              summary_line_index, 0, {virt_text = chunks})
        end
        ...
        vim.api.nvim_buf_set_virtual_text(self.buffer_number, self.namespace_id,
                                          summary_line_index, chunks, {})
        summary_line_index = summary_line_index +
                                 get_count_of_edits_diff_lines(
                                     text_document_edit)
    end
end
