...
function BaseWindow:get_content() return {} end

function BaseWindow:update_virtual_text() return end

function BaseWindow:update_buffer_content()
    vim.api.nvim_buf_clear_namespace(self.buffer_number, self.namespace_id, 0,
                                     -1)

    local content = vim.tbl_map(function(v) return v:gsub("\n", " ") end,
                                self:get_content())

    -- Unset and set the filtype option removes temporally th read-only property
    vim.api.nvim_set_option_value('filetype', '', {buf = self.buffer_number})
    vim.api.nvim_buf_set_lines(self.buffer_number, 0, -1, false, content)
    vim.api.nvim_set_option_value('filetype', self.filetype,
                                  {buf = self.buffer_number})
...
function BaseWindow:open(window_configuration_options)
    vim.validate({
        ['window configuration options'] = {
            window_configuration_options, 'table', true
        }
    })

    if self.buffer_number == -1 then self:create_buffer() end
    ...
    if self.window_number == -1 then
        self.window_number = vim.api.nvim_open_win(self.buffer_number,
                                                   self.focusable,
...
function BaseWindow:after_opened() return end
