...
local function get_stack_direction(window_stack)
    if #window_stack == 1 then return decide_for_direction(window_stack[1]) end
...
function StackingWindow:get_window_configuration(window_configuration_options)
    ...
    vim.validate({
        ['window configuration options'] = {
            window_configuration_options, 'table'
        }
    })
    vim.validate({
    ...
    vim.validate({
        ['use buffer width'] = {
            window_configuration_options.user_buffer_width, 'boolean', true
        }
    })
    ...
    if stack_direction == WindowStackDirectionEnum.UPWARDS then
        window_row = last_window:get_option('row') - window_height -
                         border_height
    ...
    elseif stack_direction == WindowStackDirectionEnum.DOWNWARDS then
        window_row = last_window:get_option('row') +
                         last_window:get_option('height') + border_height
    end
...
function StackingWindow:after_opened()
    ...
    if not last_window.is_anchor then
        ...
        local last_width = buffer_utils.get_buffer_width(
                               last_window.buffer_number)
