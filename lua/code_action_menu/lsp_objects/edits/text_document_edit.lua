...
local function uri_has_custom_scheme(uri) return uri:sub(1, 4) ~= 'file' end

local function read_line_from_file(file_name, row)
    local file_descriptor = vim.loop.fs_open(file_name, 'r', 438)

    if not file_descriptor then return inaccessible_content_placeholder end

    local file_statistics = vim.loop.fs_fstat(file_descriptor)
    if file_statistics then
        local file_content = vim.loop.fs_read(file_descriptor,
                                              (file_statistics.size or 0), 0)
...
local function get_line(uri, row)
    ...
    if vim.fn.bufloaded(file_name) == 1 then
        ...
        local lines = vim.api.nvim_buf_get_lines(buffer_number, row, row + 1,
                                                 false)
...
local function get_deleted_line_count_of_all_edits(all_edits)
    ...
    for _, edit in ipairs(all_edits) do
        ...
        -- of just one line.
        if not (startLine == endLine and startColumn == endColumn and
            text:sub(-1) == '\n') then
...
local function get_list_of_added_lines_in_edit(uri, edit)
    ...
    local first_original_complete_line =
        get_line(uri, edit.range.start.line) or ''

    local text_before_changes = first_original_complete_line:sub(0, edit.range
                                                                     .start
                                                                     .character)
    added_lines[1] = text_before_changes .. added_lines[1]

    local last_original_complete_line = get_line(uri, edit.range['end'].line) or
                                            ''
    ...
    if #last_original_complete_line > edit.range['end'].character then
        ...
        added_lines[#added_lines] = added_lines[#added_lines] ..
                                        text_after_changes
    end
...
function TextDocumentEdit:new(server_data)
    ...
    local uri = server_data.uri or server_data.newUri or
                    server_data.textDocument.uri
    local edits = server_data.edits or {}
    local kind = server_data.kind
    local status = (kind == 'create' and TextDocumentEditStatusEnum.CREATED or
    ...
    local instance = {uri = uri, edits = edits, status = status}
...
function TextDocumentEdit:get_line_number_statistics()
    ...
    elseif self.status == TextDocumentEditStatusEnum.DELETED then
        ...
        local added_line_count = get_added_line_count_of_all_edits(self.edits)
        local deleted_line_count = get_deleted_line_count_of_all_edits(
                                       self.edits)
...
function TextDocumentEdit:get_diff_lines()
    ...
    for _, edit in ipairs(self.edits) do
        ...
        local context_before_lines = get_list_of_original_lines(self.uri,
                                                                start_line -
                                                                    context_line_count,
                                                                start_line - 1)
        local deleted_lines = get_list_of_original_lines(self.uri, start_line,
                                                         end_line)
        local added_lines = get_list_of_added_lines_in_edit(self.uri, edit)
        local context_after_lines = get_list_of_original_lines(self.uri,
                                                               end_line + 1,
                                                               end_line +
                                                                   context_line_count)
